...
export class AuthController {
  constructor(private readonly authService: AuthService, private readonly logService: LogService, private readonly twilioService: TwilioService,) { }
  ...
  async validateSession(@Req() req: Request, @Res() res: Response) {
    try {
      ...
      return res.status(200).json({ message: 'Sesión válida', role: userData.role, name: userData.name });
    } catch (error) {
  ...
  async verifyOTP(@Body() body: { to: string, code: string }) {
    ...
    try {
      ...
      const verificationResult = await this.twilioService.checkVerification(to, code);
